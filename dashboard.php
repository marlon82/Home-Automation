...
if ($XS1Online){
	for( $i = 0; $i <= 64; $i++ ){
		...
		$sql = query( "SELECT * FROM aktor WHERE iName = '" . $name . "' ORDER BY name ASC" );
		$row = fetch( $sql );
	
		$sql1 = query( "SELECT id,name FROM rooms WHERE id = '" . $row['room']. "' ORDER BY name ASC" );
		$room = fetch( $sql1 );
	
		$sql2 = query( "SELECT id,devtypename,devtype FROM deviceTypes WHERE id = '" . $row['type']. "' ORDER BY devtypename ASC" );
		$devtype = fetch( $sql2 );
		
		if ($devtype['devtype'] != 'infrarot') 
		{
					...
					for( $i = 0; $i <= 64; $i++ ){
						...
						if ($devtype['devtype'] != 'infrarot') 
						{
							if( $row['id']  && $name && $type != 'disabled' )
							{
								if( $value > 1 ){
									?>
									<li><a href="#" id="invertAktor-<?php echo $row['iid'] ?>" value="off"><?php echo $room['name'] . " - " . $row['name'] . " (" . $devtype['devtypename'] . ")"; ?></a></li>
									<?php
									$count = $count + 1;
								}
							}
